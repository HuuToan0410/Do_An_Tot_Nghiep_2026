...
import {
  Search, SlidersHorizontal, ChevronLeft, ChevronRight, X, Car,
} from "lucide-react";
...
export default function VehiclesPage() {
  ...
  const hasActiveFilter =
    ...
    !!filters.fuel_type || !!filters.transmission ||
    !!filters.year || !!filters.body_type;

  function resetAllFilters() {
    setFilters({
      search: "", brand: "", min_price: "", max_price: "",
      fuel_type: "", transmission: "", year: "", body_type: "",
      ordering: "", page: 1,
    });
  }
  ...
  return (
    <MainLayout>
      {/* ── Hero header ── */}
      <section className="bg-gradient-to-r from-gray-900 to-[#111] text-white py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <p className="text-red-400 text-sm font-bold uppercase tracking-widest mb-1">Kho xe</p>
              <h1 className="text-3xl md:text-4xl font-black leading-tight">Xe Đang Bán</h1>
              <p className="text-gray-400 text-sm mt-1.5 flex items-center gap-1.5">
                <Car size={14} />
                {isLoading ? "Đang tải..." : `${data?.count ?? 0} xe đang có sẵn`}
              </p>
            </div>
          ...
          <div className="flex items-center gap-3">
            {hasActiveFilter && (
              <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-xs font-semibold px-3 py-2 rounded-xl">
                <span>{Object.values(filters).filter(Boolean).length - 2} bộ lọc đang áp dụng</span>
                <button onClick={resetAllFilters} className="hover:text-red-900"><X size={13} /></button>
              </div>
            )}
            <button
            ...
            >
              ...
              {hasActiveFilter && <span className="w-2 h-2 rounded-full bg-red-600 shrink-0" />}
            </button>
          </div>
          ...
          <div className="col-span-1 lg:col-span-3">
            {isLoading ? (
              ...
              <div className="text-center py-24 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                ...
                <h3 className="text-xl font-black text-gray-800 mb-2">Không tìm thấy xe phù hợp</h3>
                <p className="text-gray-500 text-sm mb-6">Hãy thử thay đổi bộ lọc hoặc từ khóa tìm kiếm</p>
                <button
              ...
              <>
                <div className={`grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5 transition-opacity duration-200 ${isFetching ? "opacity-60" : "opacity-100"}`}>
                  {data?.results?.map((v) => (
                    <VehicleCard
                      key={v.id}
                      id={v.id}
                  ...
                  <div className="flex items-center justify-center gap-2 mt-10 pt-8 border-t border-gray-100">
                    ...
                    <div className="flex gap-1.5">
                      {Array.from({ length: totalPages }, (_, i) => i + 1)
                        ...
                        .map((p, i) =>
                          p === "..." ? (
                            <span key={`dots-${i}`} className="px-2 py-2.5 text-gray-400 text-sm">…</span>
                          ) : (
                            <button
                              key={`page-${p}`}
          ...
          <div className="absolute right-0 top-0 bottom-0 w-80 bg-white shadow-2xl flex flex-col">
            <div className="flex items-center justify-between p-5 border-b">
              <div>
                ...
                {hasActiveFilter && <p className="text-xs text-red-600 font-medium mt-0.5">Đang áp dụng bộ lọc</p>}
              </div>
              <button onClick={() => setMobileFilterOpen(false)} className="text-gray-400 hover:text-gray-600 p-1.5 hover:bg-gray-100 rounded-lg transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-4 border-t space-y-2">
              <button onClick={() => setMobileFilterOpen(false)} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3.5 rounded-xl transition-colors">
                Áp dụng
              </button>
              {hasActiveFilter && (
                <button
